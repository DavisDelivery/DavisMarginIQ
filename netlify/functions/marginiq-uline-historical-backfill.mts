...
/**
 * Davis MarginIQ — Uline DAS Historical Backfill TRIGGER (v2.53.1, Phase 2 task 2)
 *
 ...
 * in marginiq-uline-historical-backfill-background, wrapped in
 * lib/checkpointed-runner.ts so it can survive Netlify's 15-min cap
 * via cursor + self-reinvoke.
 *
 * ENDPOINTS
 * =========
 *   GET  /.netlify/functions/marginiq-uline-historical-backfill
 ...
 *       reprocess: false,          // bypass uline_processed_emails idempotency
 *     }
 *
 * STATUS DOC
 * ==========
 * marginiq_config/uline_historical_backfill_status — schema documented
...
const BG_PATH = "/.netlify/functions/marginiq-uline-historical-backfill-background";
...
export default async (req: Request, context: Context) => {
  ...
  context.waitUntil(
    fetch(bgUrl, {
      ...
      console.error("uline-historical-backfill TRIGGER: bg fire failed", e?.message || e);
    })
  );
  ...
  return json({
    ...
    options: opts,
    status_doc_path: `marginiq_config/${STATUS_DOC}`,
    poll: `${url.origin}/.netlify/functions/marginiq-uline-historical-backfill`,
